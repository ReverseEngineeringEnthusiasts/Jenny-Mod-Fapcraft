...
public class BeeScreen extends GuiScreen {
   ...
   /**
    * Shows the overlay and runs {@code action} on a daemon thread with a
   ...
   public static void runOnMainThread(Runnable action) {
      isVisible = true;
      ThreadNames.createDaemonThread(1200, action);
   }
   ...
   public void onRenderGameOverlay(RenderGameOverlayEvent event) {
      if (isVisible) {
         if (event.getType() == ElementType.TEXT) {
            ...
            animTimer = animTimer + mc.getTickLength() * 0.75F;
            int guiScale = mc.gameSettings.guiScale;
            float offsetX;
            if (guiScale == 1) {
               offsetX = (float)RotationHelper.lerpDouble(-1800.0, 1000.0, 0.5 * Math.cos(animTimer / 25.0) + 0.5);
            } else if (guiScale == 2) {
               offsetX = (float)RotationHelper.lerpDouble(-900.0, 750.0, 0.5 * Math.cos(animTimer / 25.0) + 0.5);
            } else {
               offsetX = (float)RotationHelper.lerpDouble(-900.0, 600.0, 0.5 * Math.cos(animTimer / 25.0) + 0.5);
            }
            ...
            mc.renderEngine.bindTexture(TRADE_TEXTURE);
            this.drawTexturedModalRect(offsetX, 0.0F, 0, (int)(animTimer * 1.5), 256, 256);
            this.drawTexturedModalRect(offsetX, 256.0F, 0, (int)(animTimer * 1.5), 256, 256);
            this.drawTexturedModalRect(offsetX, 512.0F, 0, (int)(animTimer * 1.5), 256, 256);
            mc.renderEngine.bindTexture(MIRROR_TEXTURE);
            this.drawTexturedModalRect(offsetX + 600.0F, 0.0F, 0, (int)(animTimer * 1.5), 256, 256);
            this.drawTexturedModalRect(offsetX + 600.0F, 256.0F, 0, (int)(animTimer * 1.5), 256, 256);
            this.drawTexturedModalRect(offsetX + 600.0F, 512.0F, 0, (int)(animTimer * 1.5), 256, 256);
            mc.renderEngine.bindTexture(BLANK_TEXTURE);
