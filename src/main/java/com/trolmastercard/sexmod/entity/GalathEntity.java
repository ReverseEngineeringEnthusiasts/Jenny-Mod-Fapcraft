...
public class GalathEntity extends BaseGirlEntity implements IEntityMultiPart, IGalath {
   ...
   public void updateAITasks() {
      if (this.despawned) {
         ...
         this.P_clash661();
         super.updateAITasks();
         this.watchClosestGirlGoal.isWatching = this.x_clash662();
         if (this.k_clash637()) {
            this.ae_clash664();
         } else {
            this.an();
               ...
               if (!var3.bU) {
                  if (!var2.world.isRemote) {
                     if (!var3.k_clash637()) {
                        ...
                        PacketHandler.networkWrapper.sendToAllTracking(new SpawnEnergyBallParticlesPacket(var3.getGirlId(), GirlSavedData.b_clash851(var3)), var3);
